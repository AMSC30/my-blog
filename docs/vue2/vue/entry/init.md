...
可以看到，目录结构文件有很多，而且包含的面也非常杂，但只需要关心的几个部分做介绍：
...
再来看入口文件，其实入口文件这里才是`Vue`构造函数庐山真面目：
...
，`initMixin()`方法的主要作用就是在`Vue.prototype`上定义一个`_init()`实例方法
