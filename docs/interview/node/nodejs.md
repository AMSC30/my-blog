...
在浏览器外运行 V8 JavaScript 引擎（Google Chrome 的内核），利用事件驱动、非阻塞和异步IO模型等技术提高性能
...
在浏览器中，您所做的大部分时间都是与DOM或其他Web平台API（如Cookie）进行交互。当然，这些在Node.js中不存在

**环境控制不同**

除非你正在构建一个任何人都可以在任何地方部署的开源应用程序，否则你知道你将在哪个版本的Node.js上运行应用程序。与浏览器环境相比，在浏览器环境中，你无法选择访问者将使用什么浏览器

**语法差异**

在node.js中，可以使用最新的ES语法，而不需要打包工具对代码进行进行转换；同时在node.js中可以使用ESModule和Commonjs模块规范，在浏览器中只能使用ESModule规范
